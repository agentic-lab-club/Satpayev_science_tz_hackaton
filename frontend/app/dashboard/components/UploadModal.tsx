...
export function UploadModal({ isOpen, onClose }: UploadModalProps) {
  ...
  const handleFile = useCallback((incoming: File) => {
    ...
    if (!isAllowed(incoming)) {
      ...
      setFileError(`Формат .${ext || "?"} не поддерживается. Разрешены PDF, DOC, DOCX и TXT.`);
      return;
    }
    ...
    setFile(incoming);
    setProjectTitle((current) => current || incoming.name.replace(/\.[^.]+$/, ""));
    setStage("idle");
    setProgress(0);
  }, []);
  ...
  const startUpload = async () => {
    if (!file || !projectTitle.trim() || !projectType || stage !== "idle") {
      return;
    }
    ...
    try {
      ...
      const selectedType = PROJECT_TYPES.find((pt) => pt.id === projectType) ?? null;
      const createdProject = await createProject({
        title: projectTitle.trim(),
        organization_name: selectedType?.label ?? null,
      });
      setCreatedProjectId(createdProject.id);
      setProgress(30);

      const asset = await uploadAsset(file, "portfolio");
      setProgress(55);

      const version = await createVersion(createdProject.id, asset.file_id);
      setCreatedVersionId(version.id);
      setStage("analyzing");
      setProgress(70);

      await analyzeVersion(createdProject.id, version.id);
      setProgress(100);
      setStage("done");

      setTimeout(() => {
        closeModal();
        router.push(`/projects/${createdProject.id}?versionId=${version.id}`);
      }, 500);
    } catch (error) {
      console.error("Upload flow failed", error);
      setFileError(error instanceof Error ? error.message : "Не удалось завершить загрузку и анализ.");
      setStage("error");
    }
  };
  ...
  const canUpload = !!file && !!projectType && projectTitle.trim().length >= 2 && stage === "idle";
          ...
          <div className="relative px-6 py-6 space-y-6">
            {stage === "idle" && (
              <p className={`text-sm ${isDark ? "text-slate-500" : "text-slate-600"}`}>
                Загрузите документ для автоматического анализа. Backend сохранит файл, создаст версию проекта и отправит её в AI-service.
              </p>
            )}
              ...
              <>
                ...
                >
                  ...
                  {!file && (
                    <div className="flex flex-col items-center justify-center py-14 px-6 select-none">
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 transition-all ${dragOver ? "bg-amber-400/15 border border-amber-400/40" : isDark ? "bg-slate-800 border border-slate-700" : "bg-white border border-slate-200 shadow-sm"}`}>
                        <svg
                      ...
                      <div className="flex items-center gap-2 mt-5">
                        {["PDF", "DOC", "DOCX", "TXT"].map((fmt) => (
                          <span
                            key={fmt}
                            className={`text-[10px] font-bold px-2 py-0.5 rounded border ${FORMAT_META[fmt.toLowerCase()]?.bg ?? (isDark ? "bg-slate-700 border-slate-600" : "bg-slate-100 border-slate-200")} ${FORMAT_META[fmt.toLowerCase()]?.color ?? (isDark ? "text-slate-400" : "text-slate-500")}`}
                          >
                            {fmt}
                          </span>
                        ))}
                ...
                {file && (
                  <div className="grid gap-3">
                    <label className={`text-sm font-semibold ${isDark ? "text-white" : "text-slate-800"}`}>
                      Название проекта
                    </label>
                    <input
                      value={projectTitle}
                      onChange={(e) => setProjectTitle(e.target.value)}
                      placeholder="Например, ТЗ по цифровой платформе"
                      className={`w-full rounded-xl px-4 py-3 border text-sm focus:outline-none focus:ring-1 transition-colors ${
                        isDark
                          ? "bg-slate-900/60 border-slate-700 text-white placeholder-slate-500 focus:border-indigo-500/50 focus:ring-indigo-500/30"
                          : "bg-white border-slate-200 text-slate-900 placeholder-slate-400 focus:border-indigo-400/50 focus:ring-indigo-400/30"
                      }`}
                    />
                  </div>
                )}
            ...
            {stage === "idle" && (
              <ProjectTypeSelect
                ...
                visibleTypes={showAllTypes ? PROJECT_TYPES : PROJECT_TYPES.slice(0, 4)}
              ...
              <div className={`rounded-2xl border p-5 space-y-5 ${isDark ? "border-slate-800 bg-slate-900/50" : "border-slate-200 bg-slate-50/50"}`}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                      {stage === "uploading" && "Создание проекта и загрузка файла..."}
                      {stage === "analyzing" && "Запуск AI-анализа документа..."}
                ...
                <div className="space-y-3 pt-1">
                  {STAGE_STEPS.map((step, index) => {
                    const order = stage === "uploading" ? 0 : stage === "analyzing" ? 1 : stage === "done" ? 2 : -1;
                    const done = order > index;
                    const active = order === index;
                    return (
                      <div key={step.key} className={`flex items-center gap-3 transition-opacity ${!done && !active ? "opacity-40" : "opacity-100"}`}>
                        <CheckIcon done={done} active={active} isDark={isDark} />
                        <div>
                          <p className={`text-xs font-medium ${isDark ? "text-white" : "text-slate-800"}`}>{step.label}</p>
                          {active && <p className={`text-[10px] ${isDark ? "text-slate-500" : "text-slate-400"}`}>{step.sub}</p>}
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className={`rounded-2xl border p-5 ${isDark ? "border-emerald-400/20 bg-emerald-400/5" : "border-emerald-100 bg-emerald-50/50"}`}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-500"}`}>Проект сохранён в backend, анализ отправлен в AI-service и результат сохранён.</p>
                  </div>
                </div>
                  ...
                  <button
                    onClick={() => {
                      if (createdProjectId && createdVersionId) {
                        closeModal();
                        router.push(`/projects/${createdProjectId}?versionId=${createdVersionId}`);
                      }
                    }}
                    className={`flex-1 text-xs font-bold rounded-xl py-2.5 transition-all active:scale-95 ${isDark ? "bg-amber-400 hover:bg-amber-300 text-slate-900" : "bg-indigo-600 hover:bg-indigo-500 text-white shadow-sm shadow-indigo-600/20"}`}
                  >
                    Открыть проект →
                  </button>
                  <button
                    onClick={clearForm}
                    className={`flex-1 text-xs font-semibold rounded-xl py-2.5 transition-all ${isDark ? "bg-slate-800 hover:bg-slate-700 text-slate-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}
                  >
                    Загрузить ещё
                  </button>
                </div>
            ...
            {stage === "error" && (
              <div className={`rounded-2xl border p-5 ${isDark ? "border-rose-400/20 bg-rose-400/5" : "border-rose-100 bg-rose-50/50"}`}>
                <p className={`text-sm font-semibold mb-2 ${isDark ? "text-white" : "text-slate-800"}`}>Ошибка загрузки</p>
                <p className={`text-xs mb-4 ${isDark ? "text-rose-300" : "text-rose-700"}`}>{fileError}</p>
                <div className="flex gap-2">
                  <button
                    ...
                    className={`text-xs font-semibold rounded-xl px-4 py-2 transition-all ${isDark ? "bg-slate-800 hover:bg-slate-700 text-slate-200" : "bg-white hover:bg-slate-100 text-slate-700 border border-slate-200"}`}
                  >
                    Попробовать снова
                  </button>
                  <button
                    onClick={clearForm}
                    className={`text-xs font-semibold rounded-xl px-4 py-2 transition-all ${isDark ? "bg-rose-400/20 hover:bg-rose-400/30 text-rose-200" : "bg-rose-600 hover:bg-rose-500 text-white"}`}
                  >
                    Сбросить
                  </button>
                </div>
            ...
            {stage === "idle" && (
              <div className="pt-1">
                ...
                >
                  {!file ? "Сначала выберите файл" : !projectType ? "Выберите тип проекта" : !projectTitle.trim() ? "Укажите название проекта" : "Запустить AI-анализ"}
                </button>

                {canUpload && (
                  <p className={`text-[11px] text-center mt-2 ${isDark ? "text-slate-600" : "text-slate-400"}`}>
                    Backend создаст проект, загрузит файл и вернёт сохранённый анализ.
                  </p>
                )}
              </div>
            )}
